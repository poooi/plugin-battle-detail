...
const OverviewArea: React.FC<{ simulator: any }> = ({ simulator }) => {
  ...
  return (
    <div id="overview-area">
      <Card>
        ...
        >
          <h5 className="bp3-heading" style={{ margin: 0, flex: 1 }}>{__('Battle Overview')}</h5>
...
interface FleetViewProps {
  fleet: any[]
  title: string
  View?: React.FC<{ child: any }>
}

const FleetView: React.FC<FleetViewProps> = ({ fleet, title, View = ShipView }) => {
      ...
      <div>
        {rows.map(([a, b], i) => (
          <div key={i} style={{ display: 'flex' }}>
            <div style={{ flex: 1 }}><View child={a} /></div>
            <div style={{ flex: 1 }}><View child={b} /></div>
          </div>
        ))}
      </div>
    </div>
...
const ShipView: React.FC<{ child: any }> = ({ child: ship }) => {
  ...
  return (
    <div className="ship-view" style={{ display: 'flex' }}>
      ...
      <div style={{ flex: '0 0 60%' }}>
        {(data.poi_slot || []).map((item: any, i: number) => (
          <ItemView
...
const LBACView: React.FC<{ child: any }> = ({ child: corps }) => {
  ...
  return (
    <div className="lbac-view" style={{ display: 'flex' }}>
      ...
      <div style={{ flex: '0 0 60%' }}>
        {corps.api_plane_info.map((plane: any, i: number) => (
          <ItemView
